...
plot_heatmap <- function(X, y_groups = NULL, x_groups = NULL,
  ...
  if (ytext_num) {
    X_long <- X_long |>
      ...
      tidyr::pivot_longer(cols = -y, names_to = "x", values_to = "fill")
  } else {
    X_long <- X_long |>
  ...
  if (xtext_num) {
    ...
    X_long <- X_long |>
      dplyr::mutate(x = factor(x, levels = xtext_labels))
  }
  ...
  if (!is.null(x_groups)) {
    X_long$x.group <- X_long |>
      dplyr::left_join(
        data.frame(x = xtext_labels, x.group = x_groups),
        by = "x"
      ) |>
      dplyr::pull(x.group)
  }
  if (!is.null(y_groups)) {
    X_long$y.group <- X_long |>
      dplyr::left_join(
        data.frame(y = ytext_labels, y.group = y_groups),
        by = "y"
      ) |>
      dplyr::pull(y.group)
  }
  ...
  if (text_size > 0) {
    plt <- plt +
      ggplot2::geom_text(
        ggplot2::aes(x = x, y = y, label = fill),
  ...
  if (!color_by_quantile | is.factor(X_long$fill)) {
    ...
    plt <- plt +
      ggplot2::scale_fill_gradientn(
        values = heat_pal_values, colors  = heat_pal
      ) +
  ...
  plt <- plt |>
    add_axis_text_colors(
      labels = ytext_labels, label_colors = ytext_colors, axis = "y"
    ) |>
    add_axis_text_colors(
      labels = xtext_labels, label_colors = xtext_colors, axis = "x"
    )
...
plot_hclust_heatmap <- function(X, y_groups = NULL, x_groups = NULL,
  ...
  if (clust_x) {
    order_idx <- get_clust_order(
      dist_matrix_x, dist_metric_x, linkage_x,
  ...
  if (clust_y) {
    order_idx <- get_clust_order(
      dist_matrix_y, dist_metric_y, linkage_y,
  ...
  plt <- plot_heatmap(
    X = X, y_groups = y_groups, x_groups = x_groups,
